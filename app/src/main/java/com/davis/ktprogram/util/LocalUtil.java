...
import com.davis.sdj.activity.ShopActivity;
import com.davis.sdj.model.Address;
import com.davis.sdj.model.Shop;
...
public class LocalUtil implements AMapLocationListener {
    ...
    public synchronized String getLocationStr(AMapLocation location) {
        ...
        if (location.getErrorCode() == 0) {
            ...
            if(LocalUtil.getShopid(latLng, AppApplication.shoplist)){
                SharePreferenceUtils.Companion.getSharedPreferences().putString("address",location.getPoiName());
    ...
    public static boolean getShopid(LatLng point, List<Shop> list) {
        ...
        for (Shop shop : list) {
            if (shop.polygon!=null&&PtInPolygon(point, createDDRectangle(shop.polygon))) {
                ...
                SharePreferenceUtils.Companion.getSharedPreferences("kph").putString("shopid", shop.id);
                return true;
            }
        }
        return false;
